...
class Blackboard:
    ...
    def set_content(self, index: str, new_content: str):
        with self.lock:
            ...
            # we take the time at each server when the desired number of messages 
            # is reached and subtract the smallest start value (saved in code below)
            # from the largest value here to compute latency for the evaluation
            if self.counter == 10:
                f = open("time.txt", "a")
                f.write("stop" + str(time.time()) + "\n")
                f.close()
        return
...
class Server(Bottle):
    ...
    def __init__(self, ID, IP, servers_list):
        ...
        self.concurrent_posting_started = False
    ...
    def start_leader_election(self):
        ...
        # for evaluation of leader election latency (same idea as for concurrent posting latency evaluation)
        f = open("timeelection.txt", "a")
        f.write("start election " + str(time.time()) + "\n")
        f.close()
    ...
    def post_leader_election(self):
        ...
        if action == 'election':
            ...
            if self.ip != init_ip:
                ...
                # for evaluation of leader election latency (same idea as for concurrent posting latency evaluation)
                f = open("timeelection.txt", "a")
                f.write("leader elected " + str(time.time()) + "\n")
                f.close()
    ...
    # post on ('/board')
    def post_board(self):
        # we save the start time of the concurrent posting in a file
        # and subtract the smallest when all postings are fully distributed
        if (not self.concurrent_posting_started):
            f = open("time.txt", "a")
            f.write("start" + str(time.time()) + "\n")
            f.close()
            concurrent_posting_started = True
