...
class Server(Bottle):
    ...
    def start_leader_election(self):
        # put ip and random number in str separated by "|" and forward to next server
...
def main():
    ...
    try:
        server = Server(server_id,
                        server_ip,
                        servers_list)
        server.do_parallel_task_after_delay(delay=4, method=server.start_leader_election, args=[])
        bottle.run(server,
                   host=server_ip,
                   port=PORT)
    except Exception as e:
