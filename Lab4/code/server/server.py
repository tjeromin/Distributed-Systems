...
# ------------------------------------------------------------------------------------------------------
class Server(Bottle):
    legitimate = True       # set to False when node becomes byzantine
    vote_counter = 0        # keep track of number of incoming votes, send vote vector after all arrived
    result_string = ""      # will be displayed on the webpage
    ...
    # get on ('/vote/result')
    def get_vote(self):
        return template('server/templates/vote_result_template.tpl', s=Server.result_string)

    # post to ('/propagate/round1')
    def post_propagate1(self):
        Server.vote_counter += 1
                ...
                for server_no in range(no_loyal):
                    ...
                    self.do_parallel_task(self.contact_another_server,
                                          args=(self.servers_list[server_no],
                                                ...
                                                {'vote_vector': json.dumps(vector_list[server_no - own_id_passed]), 'id': self.id}))
    ...
    def post_propagate2(self):
        ...
        self.index_list.append(int(from_id) - 1)
        self.vector_list.append(json.loads(vector))

        if len(self.vector_list) == (no_total - 1) and Server.legitimate:
            ...
            Server.result_string = "result vector: " + str(result_vector) + "\nresult: " + str(result)
